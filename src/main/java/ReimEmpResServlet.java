...
@WebServlet("/ReimEmpResServlet")
public class ReimEmpResServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;

	private PostgresConnector connector;
	private ReimbursementService reimbursementService;

	public ReimEmpResServlet() {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		ReimEmpResServlet app = new ReimEmpResServlet();

		HttpSession session = request.getSession();

		String username = (String)(session.getAttribute("user"));

		List<Reimbursement> reimbursements = app.reimbursementService.getEmpResAll(username);
