...
@WebServlet("/ReimUpdateServlet")
public class ReimUpdateServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;

	private PostgresConnector connector;
	private ReimbursementService reimbursementService;

	public ReimUpdateServlet() {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		ReimUpdateServlet app = new ReimUpdateServlet();
		...
		//String reimOwner = request.getParameter("owner");
		String reimResolver = username;//request.getParameter("resolver");
		//double reimAmount = Double.parseDouble(request.getParameter("amount"));
		String reimStatus = request.getParameter("status");
		//app.reimbursementService.updateReim(reim_id, reimOwner, "reimResolver", reimAmount, reimStatus);
		app.reimbursementService.reimUpdate(reim_id, reimResolver, reimStatus);

//		response.setContentType("text/html");
//		PrintWriter out = response.getWriter();
//		out.println("<a href='index.html'>Back</a><br>");
		
		response.sendRedirect("/ReimbursementSystem/reimbursements.html");
